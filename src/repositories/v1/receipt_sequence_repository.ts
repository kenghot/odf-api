...
@EntityRepository(ReceiptSequence)
class ReceiptSequenceRepository extends Repository<RequestSequence> {
  async findSequencies(
    qs: ISequenceQuery,
    relations?: string[]
  ): Promise<[RequestSequence[], number]> {
    const { currentPage = 1, perPage = 9999, ...searchConditions } = qs;
...
export default getCustomRepository(ReceiptSequenceRepository);
